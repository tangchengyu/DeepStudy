...
const electronAPI = {
  ...
  getCurrentVersion: () => ipcRenderer.invoke("app:update-current-version"),
  ...
  syncTurnstileVerify: (input) => ipcRenderer.invoke("sync:turnstile-verify", input),
  ...
  syncConflictStatus: (conflictId, expected) => ipcRenderer.invoke("sync:conflict-status", conflictId, expected),
  syncReconcileConflict: (input, expected) => ipcRenderer.invoke("sync:conflict-reconcile", input, expected),
