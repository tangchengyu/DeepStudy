(() => {
  ...
  const zhSteps = [
    ...
    },
    {
      ...
      title: "统一设置入口",
      description: "顶部“设置”是统一入口。进入设置面板后，左侧选择子栏目，右侧完成对应设置：常规与语言用于切换界面语言；新建 API 配置用于填写模型连接并一键测试；每日任务 AI 与长期任务 AI 用于选择已保存 API 和调整个性化提示词；灵魂按摩间用于维护好句库；检查更新用于从 GitHub Release 获取新版安装包；使用教程可随时重新打开本引导。",
      view: "gate",
    },
    {
      target: "#app-settings-open",
    ...
    },
    {
      ...
      title: "账号同步与多端数据",
      description: "在 Windows、Mac 和手机上登录同一个账号，保留默认同步服务地址即可。首次登录需要人机验证；注册后的恢复码只显示一次，请离线保存。每台新设备完成一次“首次同步”：先预览本机和账号的数据数量，再确认合并；空白设备会下载账号已有数据。此后保存修改会自动上传，空闲时约每分钟检查更新，回到软件或恢复联网时会立即同步；离线数据会保留并在联网后补传，无需反复点“立即同步”。切换设备前可在账号同步中确认最近同步时间。同一条记录出现冲突时，比较本机和云端版本后选择保留；必要时可用备份编号恢复。每日计划、长期任务、专注记录、分心记录、时间审计、每日反思和灵魂按摩间里的好句子都会纳入同步。",
      view: "gate",
    },
    {
