(() => {
  ...
  const zhSteps = [
    ...
    },
    {
      target: "#long-tasks-open",
      title: "长期任务的操作方式",
      description: "长期任务左侧色块是拖拽控制区，可在象限之间移动或调整同象限顺序；点击任务卡片其他区域会进入详情编辑。右键菜单包含“复制到今日任务”和“删除”。详情页可直接编辑任务名称和支持 Markdown 渲染的备注，也可以把本地图片复制后粘贴进备注，系统会自动保存在本机。",
      view: "gate",
    },
    {
  ...
  const enSteps = [
    ...
    { target: "#long-tasks-open", title: "Long Task Operations", description: "The colored left strip is the drag handle. Drag tasks across quadrants or reorder within one quadrant. Click the rest of the card to edit details. The right-click menu includes Copy to Today and Delete. In details, paste a copied local image directly into the Markdown notes; it is stored on this computer.", view: "gate" },
