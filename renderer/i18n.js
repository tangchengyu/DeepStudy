(() => {
  ...
  const dictionaries = {
    "zh-CN": {
      ...
      helpText: "教程会介绍每日任务、长期任务、右键菜单、复选框完成逻辑、AI 设置、API 配置与账号同步。账号同步会覆盖首次同步、恢复码、冲突处理、备份恢复，以及灵魂按摩间好句子的同步范围。",
      openGuide: "打开使用教程",
      alertTitle: "提示",
      gotIt: "知道了",
    ...
    "en-US": {
      ...
      helpText: "The guide explains daily tasks, long tasks, right-click menus, checkbox completion, AI settings, API setup, and account sync, including first sync, recovery code, conflict handling, backup restore, and Quote Room sentence sync.",
